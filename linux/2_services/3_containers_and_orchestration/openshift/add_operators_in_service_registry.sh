#OpenShift Offline Operator Catalogue Build and Mirror
#git clone https://github.com/arvin-a/openshift-disconnected-operators.git

python3 mirror-operator-catalogue.py \
--catalog-version 1.0.0 \
--authfile /root/pull2.txt \
--registry-olm v00opshift08tst.ocp4.corp.domain.ru:5000 \
--registry-catalog v00opshift08tst.ocp4.corp.domain.ru:5000 \
--operator-file ./offline-operator-list \
--icsp-scope=namespace


#Перенос операторов в сервисный регистри
https://docs.openshift.com/container-platform/4.4/operators/olm-restricted-networks.html

#автодополнение табом команды "oc" 
oc completion bash | sudo tee /etc/bash_completion.d/openshift > /dev/null


podman login --authfile pull2.txt registry.redhat.io


#построили каталог с описанием (кнопочки в вебке)
oc adm catalog build \
    --appregistry-org redhat-operators \
    --from=registry.redhat.io/openshift4/ose-operator-registry:v4.4 \
    --filter-by-os="linux/amd64" \
    --to=v00opshift08tst.ocp4.corp.domain.ru:5000/olm/redhat-operators:v1 \
    -a ${REG_CREDS} \
    --insecure 


#запретить все внешние источники для OperatorHub
oc patch OperatorHub cluster --type json -p '[{"op": "add", "path": "/spec/disableAllDefaultSources", "value": true}]'


#скачать образы операторов (смирорить операторХаб)
oc adm catalog mirror v00opshift08tst.ocp4.corp.domain.ru:5000/olm/redhat-operators:v1 v00opshift08tst.ocp4.corp.domain.ru:5000  -a /root/pull2.txt --insecure --skip-verification=true

[root@v00opshift08tst openshift-disconnected-operators]# oc get is postgresql -n openshift -o json | jq .spec.tags[].from.name | grep registry.redhat.io
"registry.redhat.io/rhscl/postgresql-10-rhel7:latest"
"registry.redhat.io/rhscl/postgresql-12-rhel7:latest"
"registry.redhat.io/rhscl/postgresql-96-rhel7:latest"


oc image mirror -a /root/pull2.txt registry.redhat.io/rhscl/postgresql-10-rhel7:latest v00opshift08tst.ocp4.corp.domain.ru:5000/rhscl/postgresql-10-rhel7:latest
oc image mirror -a /root/pull2.txt registry.redhat.io/rhscl/postgresql-12-rhel7:latest v00opshift08tst.ocp4.corp.domain.ru:5000/rhscl/postgresql-12-rhel7:latest
oc image mirror -a /root/pull2.txt registry.redhat.io/rhscl/postgresql-96-rhel7:latest v00opshift08tst.ocp4.corp.domain.ru:5000/rhscl/postgresql-96-rhel7:latest

oc create configmap registry-config --from-file=v00opshift08tst.ocp4.corp.domain.ru..5000=/opt/registry/certs/v00opshift08tst.ocp4.corp.domain.ru.cer -n openshift-config
